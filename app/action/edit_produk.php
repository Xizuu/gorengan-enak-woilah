...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($produk && $harga && !$error) {
        $stmt = mysqli_prepare($koneksi, "UPDATE produk SET nama_produk = ?, harga = ?, gambar = ? WHERE id = ?");
        mysqli_stmt_bind_param($stmt, "sisi", $produk, $harga, $fotoPath, $id);
        if (mysqli_stmt_execute($stmt)) {
            ...
            echo "<script>alert('Produk gagal diperbarui')</script>";
            echo "<script>window.location.href = '/produk'</script>";
        }
    } elseif (!$error) {
        echo "<script>alert('Field wajib diisi')</script>";
        echo "<script>window.location.href = '/produk/edit?id=" . $id . "'</script>";
    }
}
?>
<html lang="en">
...
<body class="bg-[#121212] text-white font-sans min-h-screen flex overflow-x-hidden">
  ...
  <div class="flex-1 flex flex-col p-6 space-y-6 ml-0 sm:ml-56 w-full">
    ...
    <section class="bg-[#18181b] shadow-lg rounded-xl p-6 flex flex-col flex-1 max-w-full" style="min-width: 0">
        <div class="mb-4">
            <h2 class="text-white font-semibold text-base">Edit Produk</h2>
        </div>

        <form action="/produk/edit?id=<?= $id ?>" method="POST" enctype="multipart/form-data" class="space-y-4 max-w-xl">
            <!-- Nama Produk -->
            <div>
            ...
            <div>
                ...
                <input type="text" inputmode="numeric" name="harga" value="<?= htmlspecialchars($data["harga"]) ?>" required
                    class="w-full px-4 py-2 bg-[#121212] border border-gray-700 text-white rounded-md focus:outline-none focus:ring focus:border-blue-500 text-sm" />
            </div>
            ...
            <div>
                ...
                <?php if (!empty($data["gambar"])): ?>
                    <img src="data:image/jpeg;base64,<?= $data["gambar"] ?>" alt="Preview" class="mt-2" width="150">
                <?php endif; ?>
            </div>
